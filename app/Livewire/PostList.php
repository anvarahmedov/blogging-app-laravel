...
class PostList extends Component
{
    ...
    ->withCount('likes') // Ensure likes_count exists in the result set
    ->when($this->popular, function ($query) {
        $query->orderBy('likes_count', 'desc'); // Now it's defined
    })
    ->orderBy('published_at', $this->sort)
    ->where('title', 'ILIKE', "%{$this->search}%") // Case-insensitive for PostgreSQL
    ->paginate(3);
    }
